...
- [ ] The `collected/` directory structure is defined: one subdirectory per `ashuangiras/{repo}`, one file per `{commit-sha}-{control-id}-{timestamp}.yaml`
...
repository:
  ...
  url: https://github.com/ashuangiras/platform-compliance
  type: platform-repo
declared_profiles:
  - PROF-PLATFORM-V1
technology_contexts:
